%% FE assembly of 2D offshore wind turbine on jacket support structure
% including a simplified foundation model 
%
%  Authors: Pim van der Male & Dominik Fallais

% Edited by @David and @Frouwko. Modifications can be found with cntr+F and
% Edit
% -------------------------------------------------------------------------
...
%    |  |hseg      |   /  \   |          (leg 1) 
%    |  |          |/        \|        
%    -  -      x---+----------+---x  --> bottom member, 2 hor. soil springs     
%				   |		  |      --> 2 vertical soil spring					
...
% Foundation 
% ----------
% % segment numbers for foundation == last segment number +1 // +2
% sfnd = [max(find(Sections(:,1)))+1  max(find(Sections(:,1)))+2];
% %suggested better code than above
sfnd = [find(Sections(:,1),1,'last')+1  find(Sections(:,1),1,'last')+2]; 
...
% steel properties
% ----------------
Est = 209.5e9;                                                                                                        % Young's modulus steel [N/m^2]
...
% soil properties
% ---------------
Esoil = 1.744e9;          %%CHANGED                                                                                              % A Young's modulus of soil [N/m^2] 
...
% Visualisations
% comment everything below this line if model is called multiple times

% Node and element plot
% -------------------------------------------------------------------------

% figure(); 
% set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.1, 1, 0.8]); % Edit @Frouwko bigger figure
% subplot(1,6,1);                                                     %Edit @Frouwko subplots
...
% % -------------------------------------------------------------------------
% 
% for ind = 1:6% Edit @Frouwko plots in subplots instead of loose figures
%    pos = ind/7 - 0.1 ;
%    subplot(1,6,ind,'Position',[pos 0.05 0.10 0.95])
%    animdisp(Nodes,Elements,Types,DOF,Phi(:,(ind+5)));
%    title({sprintf('Phi %d',(ind+5)),sprintf('omega = %0.2f Hz ',Omega(ind+5)/2/pi)},...
%        'FontWeight','Normal','Position',[0 0],'VerticalAlignment','top');  
% end
% % 

% print -djpeg eigenval1-5.jpg -r300
% print -djpeg eigenval6-11.jpg -r300
% % Natural frequencies
% % -------------------------------------------------------------------------
% 
...
% fprintf('FE_modelrun completed on %s\n', datestr(now,'HH:MM:SS'));  % Edit @Frouwko Line added